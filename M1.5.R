...
for (indexi in 1:nofsimu) {
  ...
  yijdata_2nd <- function(x,z,tbeta,talpha,tgamma,tdelta,zlast,eps) {
    lengthofz=length(z)
    pyij=sum(x*tbeta)+sum(z*talpha)+sum(tgamma)+sum(zlast*tdelta)+eps
  ...
  yijdata_2nd_mis <- function(x,z,tbeta,talpha,tgamma,tdelta,zlast,eps) {
    lengthofz=length(z)
    pyij=sum(x*tbeta)+sum(z*talpha)+sum(tgamma)+sum(zlast*tdelta)+eps
  ...
  yijdata_3rd <- function(x,tbeta,tgamma,tdelta,zlast,eps) {
    pyij=sum(x*tbeta)+sum(tgamma)+sum(zlast*tdelta)+eps
  ...
  yijdata_3rd_mis <- function(x,tbeta,tgamma,tdelta,zlast,eps) {
    pyij=sum(x*tbeta)+sum(tgamma)+sum(zlast*tdelta)+eps
  ...
  ydata_2nd <- function(x,z,tbeta,talpha,tgamma,tdelta,zlast,eps) {
    i=length(x)
    j=nrow(x[[1]])
    yall=matrix(0,i,j)
    ...
    for (k in 1:i) {
      for (l in 1:j) {
        yall[k,l]=yijdata_2nd(x[[k]][l,],z[[k]][l,],tbeta,talpha,tgamma,tdelta,zlast[[k]][l],eps[l])
      }
    }
    return (yall)
  }
  ...
  ydata_2nd_mis <- function(x,z,tbeta,talpha,tgamma,tdelta,zlast,eps) {
    i=length(x)
    j=nrow(x[[1]])
    yall=matrix(0,i,j)
    ...
    for (k in 1:i) {
      for (l in 1:j) {
        yall[k,l]=yijdata_2nd_mis(x[[k]][l,],z[[k]][l,],tbeta,talpha,tgamma,tdelta,zlast[[k]][l],eps[l])
      }
    }
    return (yall)
  }
  ...
  ydata_3rd <- function(x,tbeta,tgamma,tdelta,zlast,eps) {
    i=length(x)
    j=nrow(x[[1]])
    yall=matrix(0,i,j)
    ...
    for (k in 1:i) {
      for (l in 1:j) {
        yall[k,l]=yijdata_3rd(x[[k]][l,],tbeta,tgamma,tdelta,zlast[[k]][l],eps[l])
      }
    }
    return (yall)
  }
  ...
  ydata_3rd_mis <- function(x,tbeta,tgamma,tdelta,zlast,eps) {
    i=length(x)
    j=nrow(x[[1]])
    yall=matrix(0,i,j)
    ...
    for (k in 1:i) {
      for (l in 1:j) {
        yall[k,l]=yijdata_3rd_mis(x[[k]][l,],tbeta,tgamma,tdelta,zlast[[k]][l],eps[l])
      }
    }
    return (yall)
  }
  ...
  sim2 <- function(sn,nhelper1,nhelper2,nhelper3,c2,tbeta,talpha,tgamma,tdelta) {
    ...
    while (si<=sn) {
      ...
      eps_2ndhelperm=vector("list",nhelper2)
      ...
      for (b in 1:nhelper2) {
        ...
        eps_2ndhelperm[[b]]=matrix(rnorm(c2[[3]][b]*1,0,eps_sd_2nd),c2[[3]][b],1)
      }
      ...
      for (i in 1:(nhelper2/2)) {
        ...
        y_2ndhelperm[[i]]=ydata_2nd_mis(x_2ndhelperm_temp,z_2ndhelperm_temp,tbeta,talpha_2nd[[i]],tgamma_2nd[[i]],tdelta_2nd[[i]],zlast_2ndhelperm_temp,eps_2ndhelperm[[i]])
      }
      ...
      for (i in (nhelper2/2+1):nhelper2) {
        ...
        z_2ndhelperm_temp[[1]]=z_2ndhelperm[[i]]
        zlast_2ndhelperm_temp[[1]]=zlast_2ndhelperm[[i]]
        y_2ndhelperm[[i]]=ydata_2nd(x_2ndhelperm_temp,z_2ndhelperm_temp,tbeta,talpha_2nd[[i]],tgamma_2nd[[i]],tdelta_2nd[[i]],zlast_2ndhelperm_temp,eps_2ndhelperm[[i]])
      }
      ...
      eps_3rdhelperm=vector("list",nhelper3)
      ...
      for (b in 1:nhelper3) {
        ...
        eps_3rdhelperm[[b]]=matrix(rnorm(c2[[4]][b]*1,0,eps_sd_3rd),c2[[4]][b],1)
      }
      ...
      for (i in 1:(nhelper3/2)) {
        ...
        y_3rdhelperm[[i]]=ydata_3rd_mis(x_3rdhelperm_temp,tbeta,tgamma_3rd[[i]],tdelta_3rd[[i]], zlast_3rdhelperm_temp,eps_3rdhelperm[[i]])
      }
      ...
      for (i in (nhelper3/2+1):nhelper3) {
        ...
        z_3rdhelperm_temp[[1]]=z_3rdhelperm[[i]]
        zlast_3rdhelperm_temp[[1]]=zlast_3rdhelperm[[i]]
        y_3rdhelperm[[i]]=ydata_3rd(x_3rdhelperm_temp,tbeta,tgamma_3rd[[i]],tdelta_2nd[[i]],zlast_3rdhelperm_temp,eps_3rdhelperm[[i]])
      }
      ...
      lltheta <- function(theta) {
        ...
        for (c in 1:nhelper2) {
          for (b in 1:c2[[3]][c]) {
            py3ijtemp=(1/sqrt(2*pi*eps_sd_2nd^2))*exp(-(y_2ndhelperm[[c]][b]-(sum(x_2ndhelperm[[c]][b,]*betainll)+sum(z_2ndhelperm[[c]][b,]*alphainll_2nd[[c]])+sum(gammainll_2nd[[c]])))^2/(2*eps_sd_2nd^2))
            if (abs(py3ijtemp)<0.00001) {
              py3ij=py3ij+(-20)
            }
            else {py3ij=py3ij+log(py3ijtemp)}
        ...
        for (c in 1:nhelper3) {
          for (b in 1:c2[[4]][c]) {
            py4ijtemp=(1/sqrt(2*pi*eps_sd_3rd^2))*exp(-(y_3rdhelperm[[c]][b]-(sum(x_3rdhelperm[[c]][b,]*betainll)+sum(gammainll_3rd[[c]])))^2/(2*eps_sd_3rd^2))
            if (abs(py4ijtemp)<0.0001) {
              py4ij=py4ij+(-20)
            }
            else {py4ij=py4ij+log(py4ijtemp)}
  ...
  #s2.2y_all[[indexi]]=simu201804225[[1]][[13]]

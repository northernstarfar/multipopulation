...
for (indexi in 1:nofsimu) {
  ...
  #This function yijdata_m is used to generate Y_ij data for the misspecification part of the logistic model.
  ...
  yijdata_m <- function(x,z,tbeta,talpha,tgamma,tdelta,zlast) {
    pyij=exp(sum(x*tbeta)+sum(z*talpha)+sum(tgamma)+sum(zlast*tdelta))/(1+exp(sum(x*tbeta)+sum(z*talpha)+sum(tgamma)+sum(zlast*tdelta)))
    yunif=runif(1,0,1)
  ...
  #This function yijdata_m_exp is used to generate the expectation of Y_ij data for the misspecification part of the logistic model.
  ...
  yijdata_1st <- function(x,z,u,tbeta,talpha,tgamma,tdelta,zlast) {
    pyij=(exp(sum(x*tbeta)+sum(z*talpha)+sum(u*tgamma)+sum(zlast*tdelta)))/(1+(exp(sum(x*tbeta)+sum(z*talpha)+sum(u*tgamma)+sum(zlast*tdelta))))
    yunif=runif(1,0,1)
  ...
  yijdata_1st_mis <- function(x,z,u,tbeta,talpha,tgamma,tdelta,zlast) {
    pyij=(exp(sum(x*tbeta)+sum(z*talpha)+sum(u*tgamma)+sum(zlast*tdelta)))/(1+(exp(sum(x*tbeta)+sum(z*talpha)+sum(u*tgamma)+sum(zlast*tdelta))))
    yunif=runif(1,0,1)
  ...
  #This function yijdata_2nd is used to generate Y_ij data for the second sets of helper models, which comes from probit model.
  #The input x should be p dimensional, z should be q dimensional.
  #tbeta should be p dimensional.
  #talpha should be q dimensional.
  #tgamma should be r dimensional.
  #Note that even tough we said tgamma should be r dimensional. We actually can only put 1 dimensional tgamma here, as gamma is an intercept.
  
  yijdata_2nd <- function(x,z,tbeta,talpha,tgamma,tdelta,zlast) {
    pyij=(exp(sum(x*tbeta)+sum(z*talpha)+sum(tgamma)+sum(zlast*tdelta)))/(1+(exp(sum(x*tbeta)+sum(z*talpha)+sum(tgamma)+sum(zlast*tdelta))))
    yunif=runif(1,0,1)
  ...
  #This function yijdata_2nd_mis is used to generate Y_ij data for the misspecification part of the second sets of helper models, which comes from probit model.
  #The input x should be p dimensional, z should be q dimensional.
  #tbeta should be p dimensional.
  #talpha should be q dimensional.
  #tgamma should be r dimensional.
  ...
  yijdata_2nd_mis <- function(x,z,tbeta,talpha,tgamma,tdelta,zlast) {
    pyij=(exp(sum(x*tbeta)+sum(z*talpha)+sum(tgamma)+sum(zlast*tdelta)))/(1+(exp(sum(x*tbeta)+sum(z*talpha)+sum(tgamma)+sum(zlast*tdelta))))
    yunif=runif(1,0,1)
  ...
  #This function yijdata_3rd is used to generate Y_ij data for the third sets of helper models, which comes from probit model.
  #The input x should be p dimensional.
  #tbeta should be p dimensional.
  #tgamma should be r dimensional.
  #Note that even tough we said tgamma should be r dimensional. We actually can only put 1 dimensional tgamma here, as gamma is an intercept.
  
  yijdata_3rd <- function(x,tbeta,tgamma,tdelta,zlast) {
    
    pyij=(exp(sum(x*tbeta)+sum(tgamma)+sum(zlast*tdelta)))/(1+(exp(sum(x*tbeta)+sum(tgamma)+sum(zlast*tdelta))))
    yunif=runif(1,0,1)
  ...
  #This function yijdata_3rd_mis is used to generate Y_ij data for the misspecification part of the third sets of helper models, which comes from probit model.
  #The input x should be p dimensional.
  #tbeta should be p dimensional.
  #tgamma should be r dimensional.
  #tdelta should be p dimensional as it is the parameter for the quadratic term of X.
  ...
  yijdata_3rd_mis <- function(x,tbeta,tgamma,tdelta,zlast) {
  ...
  #and the input z be a list containing only 1 m*p matrix.
  ...
  ydata_m <- function(x,z,tbeta,talpha,tgamma,tdelta,zlast) {
    i=length(x)
    j=nrow(x[[1]])
    yall=matrix(0,i,j)
    ...
    for (k in 1:i) {
      for (l in 1:j) {
        yall[k,l]=yijdata_m(x[[k]][l,],z[[k]][l,],tbeta,talpha,tgamma,tdelta,zlast[[k]][l])
      }
    }
    return (yall)
  }
  ...
  ydata_1st <- function(x,z,u,tbeta,talpha,tgamma,tdelta,zlast) {
    i=length(x)
    j=nrow(x[[1]])
    yall=matrix(0,i,j)
    ...
    for (k in 1:i) {
      for (l in 1:j) {
        yall[k,l]=yijdata_1st(x[[k]][l,],z[[k]][l,],u[[k]][l,],tbeta,talpha,tgamma,tdelta,zlast[[k]][l])
      }
    }
    return (yall)
  }
  ...
  ydata_1st_mis <- function(x,z,u,tbeta,talpha,tgamma,tdelta,zlast) {
    i=length(x)
    j=nrow(x[[1]])
    yall=matrix(0,i,j)
    ...
    for (k in 1:i) {
      for (l in 1:j) {
        yall[k,l]=yijdata_1st_mis(x[[k]][l,],z[[k]][l,],u[[k]][l,],tbeta,talpha,tgamma,tdelta,zlast[[k]][l])
      }
    }
    return (yall)
  }
  
  #This function ydata_2nd is used to generate Y data for all j,i, for the second sets of helper models, which comes from probit model.
  #When using ydata_2nd to generate Y data for the 2nd sets of helper models, make sure the input x be a list containing nhelper2 m*p matrix,
  #and the input z be a list containing nhelper2 m*q matrix.
  #nhelper2 is the number of populations from the 2nd sets of the helper models.
  #tbeta is a p dimensional vector.
  #talpha is a q dimensional vector.
  #tgamma is a r dimensional vector.
  
  ydata_2nd <- function(x,z,tbeta,talpha,tgamma,tdelta,zlast) {
    i=length(x)
    j=nrow(x[[1]])
    yall=matrix(0,i,j)
    ...
    for (k in 1:i) {
      for (l in 1:j) {
        yall[k,l]=yijdata_2nd(x[[k]][l,],z[[k]][l,],tbeta,talpha,tgamma,tdelta,zlast[[k]][l])
      }
    }
    return (yall)
  }
  
  #This function ydata_2nd_mis is used to generate Y data for all j,i, for the misspecification part of the second sets of helper models, which comes from probit model.
  #When using ydata_2nd_mis to generate Y data for the misspecification part of the 2nd sets of helper models, make sure the input x be a list containing nhelper2 m*p matrix,
  #and the input z be a list containing nhelper2 m*q matrix.
  #nhelper2 is the number of populations from the 2nd sets of the helper models.
  #tbeta is a p dimensional vector.
  #talpha is a q dimensional vector.
  ...
  ydata_2nd_mis <- function(x,z,tbeta,talpha,tgamma,tdelta,zlast) {
    i=length(x)
    j=nrow(x[[1]])
    yall=matrix(0,i,j)
    ...
    for (k in 1:i) {
      for (l in 1:j) {
        yall[k,l]=yijdata_2nd_mis(x[[k]][l,],z[[k]][l,],tbeta,talpha,tgamma,tdelta,zlast[[k]][l])
      }
    }
    return (yall)
  }
  
  #This function ydata_3rd is used to generate Y data for all j,i, for the third sets of helper models only, which comes from probit model.
  #When using ydata_3rd to generate Y data for the 3rd sets of helper models, make sure the input x be a list containing nhelper3 m*p matrix,
  #nhelper3 is the number of populations from the 3rd sets of the helper models.
  #tbeta is a p dimensional vector.
  #tgamma is a r dimensional vector.
  
  ydata_3rd <- function(x,tbeta,tgamma,tdelta,zlast) {
    i=length(x)
    j=nrow(x[[1]])
    yall=matrix(0,i,j)
    ...
    for (k in 1:i) {
      for (l in 1:j) {
        yall[k,l]=yijdata_3rd(x[[k]][l,],tbeta,tgamma,tdelta,zlast[[k]][l])
      }
    }
    return (yall)
  }
  
  #This function ydata_3rd_mis is used to generate Y data for all j,i, for the misspecification part of the third sets of helper models only, which comes from probit model.
  #When using ydata_3rd_mis to generate Y data for the misspecification part of the 3rd sets of helper models, make sure the input x be a list containing nhelper3 m*p matrix,
  #nhelper3 is the number of populations from the 3rd sets of the helper models.
  #tbeta is a p dimensional vector.
  #tgamma is a r dimensional vector.
  #tdelta should be p dimensional as it is the parameter for the quadratic term of X.
  
  ydata_3rd_mis <- function(x,tbeta,tgamma,tdelta,zlast) {
    i=length(x)
    j=nrow(x[[1]])
    yall=matrix(0,i,j)
    ...
    for (k in 1:i) {
      for (l in 1:j) {
        yall[k,l]=yijdata_3rd_mis(x[[k]][l,],tbeta,tgamma,tdelta,zlast[[k]][l])
      }
    }
    return (yall)
  }
  
  #This function make.data.set is used to obtain the suitable data matrix for the glm function.
  #n is the simulation number: when using for the dataset of the main model, n should be 1; when using for the dataset
  #of the 1st sets of the helper models, n should be nhelper1; when using for the dataset of the 2nd sets of the helper models,
  #n should be nhelper2.
  ...
  #y is an n*m matrix.
  #x is a list of size n, each element in this list is a m*p matrix.
  #z is a list of size n, each element in this list is a m*q matrix.
  
  make.data.set <- function(n,m,p,q,r,y,x,z) {
    ## vector indicating which family each observation belongs
    j=rep(m,n)
    family <- rep(1:n,times=j)
  ...
  make.data.setforj_2nd <- function(m,p,q,r,y,x,z) {
    ## vector indicating which family each observation belongs
    j=rep(m,1)
    family <- rep(1:1,times=j)
  ...
  #This function make.data.setforj_3rd is used to obtain the suitable data matrix for the glm function for the populations from 3rd sets of helper models.
  #m is the observation number for each simulation, corresponding to the index i in the paper.
  #p is the length of the true beta, which is also the length of each X_ji in the paper.
  #r is the length of the true gamma, which can only be 1 here, as our gamma_j is the intercept.
  #y is an m length vector.
  #x is a m*p matrix from a list of matrix containing all m*p matrix for x, this m*p matrix stands for all observations of x from the same poppulation.
  
  make.data.setforj_3rd <- function(m,p,r,y,x) {
    ## vector indicating which family each observation belongs
    j=rep(m,1)
    family <- rep(1:1,times=j)
    ...
    data.set.out <- data.frame(cbind(family,new.y,x.values,g.values))
    colnames(data.set.out) <- c("family","Y",paste("X",1:p,sep=""),paste("G",1:r,sep=""))
  ...
  mainmodel <- function(data.set,p,q,r) {
    ...
    fm <- glm(fmla,data=data.set,family=binomial(link="logit"))
  ...
  #This function secondhelpermodel is used to obtain the MLE of the parameters of the 2nd sets of helper Probit Models.
  #The input data.set is from result generated by make.data.setforj_m_2nd function.
  #p is the length of the true beta, which is also the length of each X_ij in the paper.
  #q is the length of the ture alpha, which is also the length of each Z_ij in the paper.
  #r is the length of the true gamma.
  #Note that in the second sets of helper models, the data can be from several populations.
  #So when making dataset for the secondhelpermodel function, the n could be arbitrary. 
  
  secondhelpermodel <- function(data.set,p,q,r) {
    xnam <- paste("X",1:p,sep="")
    znam <- paste("Z",1:q,sep="")
    gnam <- paste("G",1:r,sep="")
    totalnam <- c(xnam,znam,gnam)
  ...
  #This function thirdhelpermodel is used to obtain the MLE of the parameters of the 3rd sets of helper Probit Models.
  #The input data.set is from result generated by make.data.setforj_3rd function.
  #p is the length of the true beta, which is also the length of each X_ij in the paper.
  #r is the length of the true gamma.
  #Note that in the second sets of helper models, the data can be from several populations.
  #So when making dataset for the secondhelpermodel function, the n could be arbitrary. 
  
  thirdhelpermodel <- function(data.set,p,r) {
    xnam <- paste("X",1:p,sep="")
    gnam <- paste("G",1:r,sep="")
    totalnam <- c(xnam,gnam)
  ...
  yjpre <- function(x,z,tbeta,talpha,tgamma) {
    yjpre=exp(sum(x*tbeta)+sum(z*talpha)+tgamma)/(1+exp(sum(x*tbeta)+sum(z*talpha)+tgamma))
  ...
  sim2 <- function(sn,nhelper1,nhelper2,nhelper3,c2,tbeta,talpha,tgamma,tdelta) {
    c1=1+nhelper1+nhelper2+nhelper3
    p=length(tbeta)
    q=length(talpha[[1]])
    ...
    r3=length(tgamma[[1+nhelper1+1]]) #r3 stands for the length of the 1+nhelper1+1-th element of tgamma list, which should actually always be 1.
    r4=length(tgamma[[1+nhelper1+nhelper2+1]]) #r4 stands for the length of the 1+nhelper1+nhelper2+1-th element of tgamma list, which should actually always be 1. 
    ...
    while (si<=sn) {
      ...
      y_mainm=ydata_m(x_mainm,z_mainm,tbeta,talpha[[1]],tgamma[[1]],tdelta[[1]],zlast_main)
      ...
      for (i in 1:(nhelper1/2)) {
        ...
        y_1sthelperm[[i]]=ydata_1st_mis(x_1sthelperm_temp,z_1sthelperm_temp,u_1sthelperm_temp,tbeta,talpha_1st[[i]],tgamma_1st[[i]],tdelta_1st[[i]],zlast_1sthelperm_temp)
      }
      ...
      for (i in (nhelper1/2+1):nhelper1) {
        ...
        z_1sthelperm_temp[[1]]=z_1sthelperm[[i]]
        u_1sthelperm_temp[[1]]=u_1sthelperm[[i]]
        y_1sthelperm[[i]]=ydata_1st(x_1sthelperm_temp,z_1sthelperm_temp,u_1sthelperm_temp,tbeta,talpha_1st[[i]],tgamma_1st[[i]],tdelta_1st[[i]],zlast_1sthelperm_temp)
      }
      
      #Generating list to store x,z data for the 2nd sets of the helper models, in the 2nd sets of the helper models we have nhelper2 populations, so the number of elements in these lists
      #is nhelper2.
      
      x_2ndhelperm=vector("list",nhelper2)
      z_2ndhelperm=vector("list",nhelper2)
      zlast_2ndhelperm=vector("list",nhelper2)
      xandz_2ndhelperm=vector("list",nhelper2)
      
      for (b in 1:nhelper2) {
        xandz_2ndhelperm[[b]]=rmvnorm(c2[[3]][b],MU,OMEGAcov)
        x_2ndhelperm[[b]]=xandz_2ndhelperm[[b]][,1:p]
        z_2ndhelperm[[b]]=xandz_2ndhelperm[[b]][,(p+1):(p+q)]
        zlast_2ndhelperm[[b]]=xandz_2ndhelperm[[b]][,(p+q+1):(p+q+lengthofzlast)]
      }
      
      talpha_2nd <- vector("list",nhelper2)
      tgamma_2nd <- vector("list",nhelper2)
      tdelta_2nd <- vector("list",nhelper2)
      
      for (i in 1:nhelper2) {
        talpha_2nd[[i]]=talpha[[1+nhelper1+i]]
        tgamma_2nd[[i]]=tgamma[[1+nhelper1+i]]
        tdelta_2nd[[i]]=tdelta[[1+nhelper1+i]]
      }
      
      
      y_2ndhelperm=vector("list",nhelper2)
      
      for (i in 1:(nhelper2/2)) {
        x_2ndhelperm_temp=vector("list",1)
        z_2ndhelperm_temp=vector("list",1)
        zlast_2ndhelperm_temp=vector("list",1)
        x_2ndhelperm_temp[[1]]=x_2ndhelperm[[i]]
        z_2ndhelperm_temp[[1]]=z_2ndhelperm[[i]]
        zlast_2ndhelperm_temp[[1]]=zlast_2ndhelperm[[i]]
        y_2ndhelperm[[i]]=ydata_2nd_mis(x_2ndhelperm_temp,z_2ndhelperm_temp,tbeta,talpha_2nd[[i]],tgamma_2nd[[i]],tdelta_2nd[[i]],zlast_2ndhelperm_temp)
      }
      
      for (i in (nhelper2/2+1):nhelper2) {
        x_2ndhelperm_temp=vector("list",1)
        z_2ndhelperm_temp=vector("list",1)
        zlast_2ndhelperm_temp=vector("list",1)
        x_2ndhelperm_temp[[1]]=x_2ndhelperm[[i]]
        z_2ndhelperm_temp[[1]]=z_2ndhelperm[[i]]
        zlast_2ndhelperm_temp[[1]]=zlast_2ndhelperm[[i]]
        y_2ndhelperm[[i]]=ydata_2nd(x_2ndhelperm_temp,z_2ndhelperm_temp,tbeta,talpha_2nd[[i]],tgamma_2nd[[i]],tdelta_2nd[[i]],zlast_2ndhelperm_temp)
      }
      
      #Generating list to store x data for the 3rd sets of the helper models, in the 3rd sets of the helper models we have nhelper3 populations, so the number of elements in these lists
      #is nhelper3.
      
      x_3rdhelperm=vector("list",nhelper3)
      z_3rdhelperm=vector("list",nhelper3)
      zlast_3rdhelperm=vector("list",nhelper3)
      xandz_3rdhelperm=vector("list",nhelper3)
      
      for (b in 1:nhelper3) {
        xandz_3rdhelperm[[b]]=rmvnorm(c2[[4]][b],MU,OMEGAcov)
        x_3rdhelperm[[b]]=xandz_3rdhelperm[[b]][,1:p]
        z_3rdhelperm[[b]]=xandz_3rdhelperm[[b]][,(p+1):(p+q)]
        zlast_3rdhelperm[[b]]=xandz_3rdhelperm[[b]][,(p+q+1):(p+q+lengthofzlast)]
      }
      
      tgamma_3rd <- vector("list",nhelper3)
      tdelta_3rd <- vector("list",nhelper3)
      
      for (i in 1:nhelper3) {
        tgamma_3rd[[i]]=tgamma[[1+nhelper1+nhelper2+i]]
        tdelta_3rd[[i]]=tdelta[[1+nhelper1+nhelper2+i]]
      }
      
     
      y_3rdhelperm=vector("list",nhelper3)
      
      for (i in 1:(nhelper3/2)) {
        x_3rdhelperm_temp=vector("list",1)
        z_3rdhelperm_temp=vector("list",1)
        zlast_3rdhelperm_temp=vector("list",1)
        x_3rdhelperm_temp[[1]]=x_3rdhelperm[[i]]
        z_3rdhelperm_temp[[1]]=z_3rdhelperm[[i]]
        zlast_3rdhelperm_temp[[1]]=zlast_3rdhelperm[[i]]
        y_3rdhelperm[[i]]=ydata_3rd_mis(x_3rdhelperm_temp,tbeta,tgamma_3rd[[i]],tdelta_3rd[[i]],zlast_3rdhelperm_temp)
      }
      
      for (i in (nhelper3/2+1):nhelper3) {
        x_3rdhelperm_temp=vector("list",1)
        z_3rdhelperm_temp=vector("list",1)
        zlast_3rdhelperm_temp=vector("list",1)
        x_3rdhelperm_temp[[1]]=x_3rdhelperm[[i]]
        z_3rdhelperm_temp[[1]]=z_3rdhelperm[[i]]
        zlast_3rdhelperm_temp[[1]]=zlast_3rdhelperm[[i]]
        y_3rdhelperm[[i]]=ydata_3rd(x_3rdhelperm_temp,tbeta,tgamma_3rd[[i]],tdelta_2nd[[i]],zlast_3rdhelperm_temp)
      }
      ...
      secondhelperm_out_initial=vector("list",nhelper2)
      
      for (b in 1:nhelper2) {
        x_2ndhelperm_temp=x_2ndhelperm[[b]]
        z_2ndhelperm_temp=z_2ndhelperm[[b]]
        y_2ndhelperm_temp=y_2ndhelperm[[b]]
        
        dataset_2ndhelperm_temp=make.data.setforj_m_2nd(c2[[3]][b],p,q,r3,y_2ndhelperm_temp,x_2ndhelperm_temp,z_2ndhelperm_temp)
        secondhelperm_out_initial[[b]]=secondhelpermodel(dataset_2ndhelperm_temp,p,q,r3)
      }
      
      #Extract alpha_j^hat and gamma_j^hat from the 2nd sets of helper models.
      talpha_2nd_hat=vector("list",nhelper2)
      tgamma_2nd_hat=vector("list",nhelper2)
      
      for (b in 1:nhelper2) {
        talpha_2nd_hat[[b]]=as.vector(secondhelperm_out_initial[[b]]$out.fixed[,1][(p+1):(p+q)])
        tgamma_2nd_hat[[b]]=as.vector(secondhelperm_out_initial[[b]]$out.fixed[,1][(p+q+1):(p+q+r3)])
      }
      
      thirdhelperm_out_initial=vector("list",nhelper3)
      
      for (b in 1:nhelper3) {
        x_3rdhelperm_temp=x_3rdhelperm[[b]]
        y_3rdhelperm_temp=y_3rdhelperm[[b]]
        
        dataset_3rdhelperm_temp=make.data.setforj_3rd(c2[[4]][b],p,r4,y_3rdhelperm_temp,x_3rdhelperm_temp)
        thirdhelperm_out_initial[[b]]=thirdhelpermodel(dataset_3rdhelperm_temp,p,r4)
      }
      
      #Extract alpha_j^hat and gamma_j^hat from the 3rd sets of helper models.
      talpha_3rd_hat=vector("list",nhelper3)
      tgamma_3rd_hat=vector("list",nhelper3)
      
      for (b in 1:nhelper3) {
        talpha_3rd_hat[[b]]=as.vector(thirdhelperm_out_initial[[b]]$out.fixed[,1][(p+1):(p+q)])
        tgamma_3rd_hat[[b]]=as.vector(thirdhelperm_out_initial[[b]]$out.fixed[,1][(p+q+1):(p+q+r4)])
      }
      ...
      talpha_1st2nd3rd[[2]]=talpha_2nd_hat
      talpha_1st2nd3rd[[3]]=talpha_3rd_hat
      
      tgamma_1st2nd3rd[[1]]=tgamma_1st_hat
      tgamma_1st2nd3rd[[2]]=tgamma_2nd_hat
      tgamma_1st2nd3rd[[3]]=tgamma_3rd_hat
      
      #Generate constbeta input for cvwmatrix function.
      
      constbeta=vector("list",nhelper1+nhelper2+nhelper3)
      ...
      for (b in 1:nhelper2) {
        constbeta[[b+nhelper1]]=as.vector(secondhelperm_out_initial[[b]][[1]][,1][1:p])
      }
      
      for (b in 1:nhelper3) {
        constbeta[[b+nhelper1+nhelper2]]=as.vector(thirdhelperm_out_initial[[b]][[1]][,1][1:p])
      }
      
      constbetaall=vector("list",1+nhelper1+nhelper2+nhelper3)
      
      constbetaall[[1]]=tbeta1_hat
      
      for (b in 2:(1+nhelper1+nhelper2+nhelper3)) {
      ...
      lltheta <- function(theta) {
        ...
        betainll <- theta[1:p]
        alphainll <- theta[(p+1):(p+q*(1+nhelper1+nhelper2))]
        gammainll <- theta[(p+q*(1+nhelper1+nhelper2)+1):(p+q*(1+nhelper1+nhelper2)+r1*1+r2*nhelper1+r3*nhelper2+r4*nhelper3)]
        
        alphainll_m=alphainll[1:q]
        alphainll_1st=vector("list",nhelper1)
        alphainll_2nd=vector("list",nhelper2)
        
        gammainll_m=gammainll[1:r1]
        gammainll_1st=vector("list",nhelper1)
        gammainll_2nd=vector("list",nhelper2)
        gammainll_3rd=vector("list",nhelper3)
        ...
        for (b in 1:nhelper2) {
          alphainll_2nd[[b]]=alphainll[(q*nhelper1+q*b+1):(q*nhelper1+q*b+q)]
          gammainll_2nd[[b]]=gammainll[(r1*1+r2*nhelper1+r3*b-r3+1):(r1*1+r2*nhelper1+r3*b)]
        }
        
        for (b in 1:nhelper3) {
          gammainll_3rd[[b]]=gammainll[(r1*1+r2*nhelper1+r3*nhelper2+r4*b-r4+1):(r1*1+r2*nhelper1+r3*nhelper2+r4*b)]
        }
        
        
        py1ij=0
        for (b in 1:c2[[1]][1]) {
          py1ijtemp=(exp(sum(x_mainm[[1]][b,]*betainll)+sum(z_mainm[[1]][b,]*alphainll_m)+sum(gammainll_m)))/(1+(exp(sum(x_mainm[[1]][b,]*betainll)+sum(z_mainm[[1]][b,]*alphainll_m)+sum(gammainll_m))))
          if (py1ijtemp==1) {
            py1ij=py1ij+y_mainm[1,b]*log(py1ijtemp)+(1-y_mainm[1,b])*(-20)
          }
          else {py1ij=py1ij+y_mainm[1,b]*log(py1ijtemp)+(1-y_mainm[1,b])*log(1-py1ijtemp)}
        }
        ...
        for (c in 1:nhelper1) {
          for (b in 1:c2[[2]][c]) {
            py2ijtemp=(exp(sum(x_1sthelperm[[c]][b,]*betainll)+sum(z_1sthelperm[[c]][b,]*alphainll_1st[[c]])+sum(u_1sthelperm[[c]][b,]*gammainll_1st[[c]])))/(1+(exp(sum(x_1sthelperm[[c]][b,]*betainll)+sum(z_1sthelperm[[c]][b,]*alphainll_1st[[c]])+sum(u_1sthelperm[[c]][b,]*gammainll_1st[[c]]))))
            if (py2ijtemp==1) {
              py2ij=py2ij+y_1sthelperm[[c]][b]*log(py2ijtemp)+(1-y_1sthelperm[[c]][b])*(-20)
            }
            else {
              py2ij=py2ij+y_1sthelperm[[c]][b]*log(py2ijtemp)+(1-y_1sthelperm[[c]][b])*log(1-py2ijtemp)
            }
        ...
        for (c in 1:nhelper2) {
          for (b in 1:c2[[3]][c]) {
            ...
            py3ijtemp=(exp(sum(x_2ndhelperm[[c]][b,]*betainll)+sum(z_2ndhelperm[[c]][b,]*alphainll_2nd[[c]])+sum(gammainll_2nd[[c]])))/(1+(exp(sum(x_2ndhelperm[[c]][b,]*betainll)+sum(z_2ndhelperm[[c]][b,]*alphainll_2nd[[c]])+sum(gammainll_2nd[[c]]))))
            if (py3ijtemp==1) {
              py3ij=py3ij+y_2ndhelperm[[c]][b]*log(py3ijtemp)+(1-y_2ndhelperm[[c]][b])*(-20)
            }
            else {
              py3ij=py3ij+y_2ndhelperm[[c]][b]*log(py3ijtemp)+(1-y_2ndhelperm[[c]][b])*log(1-py3ijtemp)
            }
        ...
        for (c in 1:nhelper3) {
          for (b in 1:c2[[4]][c]) {
            ...
            py4ijtemp=(exp(sum(x_3rdhelperm[[c]][b,]*betainll)+sum(gammainll_3rd[[c]])))/(1+(exp(sum(x_3rdhelperm[[c]][b,]*betainll)+sum(gammainll_3rd[[c]]))))
            if (py4ijtemp==1) {
              py4ij=py4ij+y_3rdhelperm[[c]][b]*log(py4ijtemp)+(1-y_3rdhelperm[[c]][b])*(-20)
            }
            else {
              py4ij=py4ij+y_3rdhelperm[[c]][b]*log(py4ijtemp)+(1-y_3rdhelperm[[c]][b])*log(1-py4ijtemp)
            }
            
          }
        }
        
        llthetaall=py1ij+py2ij+py3ij+py4ij
      ...
      for (b in 1:(1+nhelper1+nhelper2)) {
        thetainitial=c(thetainitial,talpha[[b]])
      }
      
      for (b in 1:(1+nhelper1+nhelper2+nhelper3)) {
  ...
  tauhat <- function(estpar) {
    ...
    tauhat_temp=sum(w1[1])+sum(w1[(1+nhelper1/2+1):(1+nhelper1)])+sum(w1[(2+nhelper1+nhelper2/2):(1+nhelper1+nhelper2)])+sum(w1[(2+nhelper1+nhelper2+nhelper3/2):(1+nhelper1+nhelper2+nhelper3)])
  ...
  c2=vector("list",4)
  c2[[1]]=c(100)                                    #c2 is the number of observations from each population.
  c2[[2]]=c(200,100)
  c2[[3]]=c(200,100)
  c2[[4]]=c(200,100)                              #c2 is the number of observations from each population.
  ...
  #parameters for the 1 of 2nd helper model.
  talpha[[4]]=c(0.02,0.05,-0.03,-0.01) 
  tgamma[[4]]=-0.7
  tdelta[[4]]=2.5
  
  #parameters for the 2 of 2nd helper model.
  talpha[[5]]=c(0.03,-0.07,0.06,0.02) 
  tgamma[[5]]=-0.72
  tdelta[[5]]=0
  
  #parameters for the 1 of 3rd helper model.
  tgamma[[6]]=-0.85
  tdelta[[6]]=2.5
  
  #parameters for the 2 of 3rd helper model.
  tgamma[[7]]=-0.87
  tdelta[[7]]=0
  
  
  simu201804225=sim2(1,nhelper1,nhelper2,nhelper3,c2,tbeta,talpha,tgamma,tdelta) #the 1st argument means how many simulations we would like to run.
  ...
  s2.2[[indexi]]=msltemp
  s2.2y_m[[indexi]]=simu201804225[[1]][[9]]
  s2.2y_1st[[indexi]]=simu201804225[[1]][[10]]
  s2.2y_2nd[[indexi]]=simu201804225[[1]][[11]]
  s2.2y_3rd[[indexi]]=simu201804225[[1]][[12]]
  s2.2estpar[[indexi]]=estpar1
  s2.2y_ob_out[[indexi]]=y_out_observe
  s2.2testdata[[indexi]]=testdata_1
  s2.2estpar[[indexi]]=estpar1
  indexi=indexi+1
}
...
which(is.nan(s2.2msl1),arr.ind = TRUE)
which(is.nan(s2.2msl2),arr.ind = TRUE)
which(is.nan(s2.2msl3),arr.ind = TRUE)
which(is.nan(s2.2msl4),arr.ind = TRUE)
which(is.nan(s2.2msl5),arr.ind = TRUE)

msl1aver=sum(s2.2msl1)/100
msl2aver=sum(s2.2msl2)/100
msl3aver=sum(s2.2msl3)/100
msl4aver=sum(s2.2msl4)/100
msl5aver=sum(s2.2msl5)/100
msl1aver
msl2aver
msl3aver
msl4aver
